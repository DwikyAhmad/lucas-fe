...
import CardFilter from "@/components/productService/cardFilter";
...
const Products = () => {
    ...
    const selectFilter = (value: string) => {
        setListFilter((prev) =>
            prev.includes(value) ? prev : [...listFilter, value]
        );
    };
    ...
    useEffect(() => {
        ...
        if (
            param == undefined ||
            param == null ||
            (param.toString() == "" && listFilter.length == 0)
        ) {
            setSearch("all");
                ...
                <div className="filter text-white bg-redBricks h-full lg:w-max w-full lg:rounded-xl rounded-full px-4 py-2 gap-2 lg:pt-5 lg:px-10 lg:pb-20 border flex lg:flex-col lg:items-start items-center align-middle justify-center ">
                    ...
                    {categories.map((category, index) => (
                        <CardFilter
                            ...
                            isChecked={false}
                            value={category.name}
                            onFilterClick={(value) => selectFilter(value)}
                        />
                    ))}
                </div>
                                ...
                                {listProduct
                                    .filter(
                                        (product) =>
                                            ...
                                            product.categoryName.includes(
                                                filter
                                            )
                                    ) // next handle by name (search)
                                    .map((product, index) =>
                                        product ? (
                                            <div
                                                className=" w-min h-max flex flex-col gap-4 mb-8  m-2  "
                                                key={index}
                                            >
                                                <div
                                                    className="text-black border md:w-max w-min flex items-center  flex-col border-black border-opacity-20 hover:shadow-xl text-3xl rounded-2xl transition-transform duration-10 ease-in-out  lg:py-1 py-0 pb-2   "
                                                    style={{
                                                        cursor: "pointer",
                                                    }}
                                                    onClick={() =>
                                                ...
                                                >
                                                    <div className="product-image border    rounded-xl drop-shadow-2xl border-black border-opacity-20 flex items-center justify-center align-middle w-auto h-max mb-2 scale-75 md:scale-100  ">
                                                        <Image
                                                            ...
                                                            className=" rounded-xl w-max  border border-black "
                                                            onClick={() =>
                                                                router?.push(
                                                                    `/products/${product.id}`
                                                                )
                                                    ...
                                                    <div className="metadata md:px-4 px-2 ">
                                                        <div className="desc md:mx-4  ">
                                                            <div className="title font-semibold font-poppins lg:text-2xl text-xl text-primaryBlueNavy">
                                                                {product.name}
                                                            </div>
                                                        ...
                                                        <div className="price flex justify-between  md:mt-8 mt-4  md:gap-4 gap-2">
                                                            ...
                                                            <div className="marketplace">
                                                                <div
                                                                    className="border border-primaryBlack rounded-xl px-2 py-1 text-2xl font-semibold h-full items-center flex bg-primaryYellow text-white transition-transform duration-300 ease-in-out hover:scale-105 md:scale-100 scale-75drop-shadow-2xl hover:bg-darkYellow "
                                                                    style={{
                                                                        cursor: "pointer",
                                                                    }}
                                                                >
                                            ...
                                            <h2
                                                key={index}
                                                className="text-2xl text-primaryBlueNavy font-semibold"
                                            >
